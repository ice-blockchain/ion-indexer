...
const char* ion_marker_decode_opcode(unsigned int opcode) {
    return to_c_str(ion_marker::decode_opcode(opcode));
}

const char* ion_marker_decode_boc(const char* boc_base64) {
    if (!boc_base64) return nullptr;
    return to_c_str(ion_marker::decode_boc(std::string(boc_base64)));
}

TonMarkerBatchResponse* ion_marker_process_batch(const TonMarkerBatchRequest* request) {
    // prepare C++ request
    ion_marker::BatchRequest cpp_request;
    ...
    for (int i = 0; i < request->boc_count; ++i) {
        ...
        ion_marker::DecodeBocRequest req;
    ...
    // process batch
    auto cpp_response = ion_marker::process_batch(cpp_request);
...
void ion_marker_free_batch_response(TonMarkerBatchResponse* response) {
    if (!response) return;
...
void ion_marker_free_string(const char* str) {
    if (str) {
        delete[] str;
    }
}
