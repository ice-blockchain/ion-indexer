...
void TonConnectProcessor::start_up() {
  LOG(INFO) << "Processing TonConnect task " << ionconnect_task_.id << " for address " << ionconnect_task_.from;
  ...
  auto addr = block::StdAddress::parse(ionconnect_task_.from);
  if (addr.is_error()) {
    error(td::Status::Error("Invalid address format: " + ionconnect_task_.from));
    return;
  }
...
void TonConnectProcessor::error(td::Status error) {
  LOG(ERROR) << "Error processing TonConnect task " << ionconnect_task_.id << ": " << error;
...
void TonConnectProcessor::got_account_state(schema::AccountState account_state) {
  ...
  trace_task.id = ionconnect_task_.id;
  trace_task.boc = boc_result.move_as_ok();
  trace_task.ignore_chksig = true;
  trace_task.detect_interfaces = ionconnect_task_.detect_interfaces;
  trace_task.include_code_data = ionconnect_task_.include_code_data;
  trace_task.mc_block_seqno = ionconnect_task_.mc_block_seqno;

  LOG(INFO) << "Successfully converted TonConnect task " << ionconnect_task_.id << " to regular TraceTask";
...
td::Result<td::Ref<vm::Cell>> TonConnectProcessor::message_to_cell(const TonConnectMessage& out_msg) {
  TRY_RESULT(src_addr, block::StdAddress::parse(ionconnect_task_.from));
...
td::Result<td::Ref<vm::Cell>> TonConnectProcessor::compose_message_body_v3(const schema::AccountState& account_state) {
  ...
  cb.store_bytes(td::Slice(dummy_signature))
    ...
    .store_long(seqno, 32);
  for (const auto &out_msg : ionconnect_task_.messages) {
...
td::Result<td::Ref<vm::Cell>> TonConnectProcessor::compose_external_message(td::Ref<vm::Cell> body) {
  ...
  /*info*/ {
    ...
    /* dest */ {
      auto addr = block::StdAddress::parse(ionconnect_task_.from).move_as_ok();
      block::gen::MsgAddressInt::Record_addr_std dest;
      dest.anycast = vm::CellBuilder().store_zeroes(1).as_cellslice_ref();
...
td::Result<td::Ref<vm::Cell>> TonConnectProcessor::compose_message_body_v4(const schema::AccountState& account_state) {
  ...
  vm::CellBuilder cb;
  auto valid_until = ionconnect_task_.valid_until.value_or(td::Timestamp::now().at_unix() + 300);
...
td::Result<td::Ref<vm::Cell>> TonConnectProcessor::compose_message_body_v5(const schema::AccountState& account_state) {
  ...
  if (ionconnect_task_.messages.empty())
    return td::Status::Error("No messages to send");
  if (ionconnect_task_.messages.size() > 255)
