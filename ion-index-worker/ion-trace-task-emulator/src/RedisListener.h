...
struct TraceEmulationResult {
  ...
  std::optional<ion::BlockId> mc_block_id;
};
...
private:
  ...
  ion::BlockId current_mc_block_id_;
...
private:
  void trace_error(std::string task_id, std::optional<ion::BlockId> mc_block_id, td::Status error);
  void trace_received(TraceTask task, MasterchainBlockDataState mc_data_state, Trace trace);
  void trace_interfaces_error(std::string task_id, ion::BlockId mc_block_id, td::Status error);
  void finish_processing(std::string task_id, ion::BlockId mc_block_id, Trace trace);
  
  // Process regular trace task
  void process_trace_task(TraceTask task);
  
  // Process TonConnect task (converts to regular trace task)
  void process_ionconnect_task(TonConnectTraceTask task);
};
