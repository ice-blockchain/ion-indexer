...
struct TaskEnvelope {
    std::string     type; // "trace" | "ionconnect"
...
public:
  TonConnectProcessor(TonConnectTraceTask ionconnect_task, const MasterchainBlockDataState& mc_data_state, td::Promise<TraceTask> promise) 
    : ionconnect_task_(std::move(ionconnect_task)), mc_data_state_(mc_data_state), promise_(std::move(promise)) {}

  // Convert TonConnect task to regular TraceTask
  void process_ionconnect_task(const TonConnectTraceTask& ionconnect_task, 
                            const MasterchainBlockDataState& mc_data_state);

  void start_up() override;
private:
  TonConnectTraceTask ionconnect_task_;
