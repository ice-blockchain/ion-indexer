...
void RedisListener::alarm() {
  while (auto buffer = redis_.rpop(queue_name_)) {
    ...
    bool is_ionconnect_task = false;
    TraceTask trace_task;
    TonConnectTraceTask ionconnect_task;
    ...
    if (env.type == "trace") {
        ...
        is_trace_task = true;
    } else if (env.type == "ionconnect") {
        env.task.convert(ionconnect_task);
        is_ionconnect_task = true;
    } else {
        LOG(ERROR) << "Unknown task type: " << env.type;
        continue;
    }

    if (is_ionconnect_task) {
        // Handle TonConnect task using the processor
        process_ionconnect_task(std::move(ionconnect_task));
        continue;
    }
...
void RedisListener::trace_error(std::string task_id, std::optional<ion::BlockId> mc_block_id, td::Status error) {
...
void RedisListener::trace_interfaces_error(std::string task_id, ion::BlockId mc_block_id, td::Status error) {
    LOG(ERROR) << "Failed to detect interfaces on task " << task_id << ": " << error;
    TraceEmulationResult res{std::move(task_id), error.move_as_error(), mc_block_id};
    trace_processor_(std::move(res), td::PromiseCreator::lambda([](td::Result<td::Unit> R) {}));
}

void RedisListener::finish_processing(std::string task_id, ion::BlockId mc_block_id, Trace trace) {
...
void RedisListener::process_ionconnect_task(TonConnectTraceTask ionconnect_task) {
  LOG(INFO) << "Processing TonConnect task " << ionconnect_task.id;
  
  if (mc_data_state_.config_ == nullptr) {
    trace_error(ionconnect_task.id, std::nullopt, td::Status::Error("RedisListener not ready"));
    return;
  }

  auto P = td::PromiseCreator::lambda([SelfId = actor_id(this), task_id = ionconnect_task.id](td::Result<TraceTask> R) {
    if (R.is_error()) {
  ...
  td::actor::create_actor<TonConnectProcessor>("TonConnectProcessor", std::move(ionconnect_task), 
