...
INDEX_ARGS=""
if [[ "$ION_WORKER_BINARY" =~ ^(ion-trace-emulator|ion-trace-task-emulator|ion-index-postgres)$ ]]; then
...
ulimit -n 1000000
echo "Running binary: $ION_WORKER_BINARY"
exec "$ION_WORKER_BINARY" $INDEX_ARGS "$@"