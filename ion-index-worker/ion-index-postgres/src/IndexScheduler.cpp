...
void IndexScheduler::process_existing_seqnos(td::Result<std::vector<std::uint32_t>> R) {
    ...
    ion::BlockSeqno next_seqno = from_seqno_;
    ...
    auto P = td::PromiseCreator::lambda([SelfId = actor_id(this), next_seqno](td::Result<ion::BlockSeqno> R) {
        if (R.is_error()) {
...
void IndexScheduler::handle_missing_ta_state(ion::BlockSeqno next_seqno) {
    LOG(WARNING) << "Trace assembler state missing for seqno " << (next_seqno - 1);

    ion::BlockSeqno backtrack_point;
...
void IndexScheduler::handle_valid_ta_state(ion::BlockSeqno last_state_seqno) {
...
void IndexScheduler::got_newest_mc_seqno(std::uint32_t newest_mc_seqno) {
    ...
    if (!is_in_sync_ && (last_known_seqno_ - last_indexed_seqno_ <= IS_IN_SYNC_THRESHOLD)) {
        LOG(INFO) << "Indexer is in sync with ION node";
        is_in_sync_ = true;
        ...
        LOG(WARNING) << "Indexing is out of sync with ION node";
        is_in_sync_ = false;
