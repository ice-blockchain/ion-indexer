...
bool InsertBatchPostgres::try_acquire_leader_lock() {
  ...
  WITH grab AS (
    ...
    ON CONFLICT(id) DO UPDATE SET
      ...
      started_at = CASE
        WHEN excluded.leader_worker_id = $1 THEN ion_indexer_leader.started_at
        ELSE now()
      END
    WHERE ion_indexer_leader.id = 1 AND (ion_indexer_leader.last_heartbeat < NOW() - INTERVAL '20 seconds'
      OR ion_indexer_leader.leader_worker_id = $1)
    RETURNING 1
  )
...
std::string InsertBatchPostgres::insert_nft_items(pqxx::work &txn) {
  std::map<std::pair<ion::StdSmcAddress, ion::StdSmcAddress>, block::StdAddress> sale_real_owners;
  std::map<std::pair<ion::StdSmcAddress, ion::StdSmcAddress>, block::StdAddress> auction_real_owners;
...
void InsertBatchPostgres::insert_jetton_transfers(pqxx::work &txn, bool with_copy) {
  std::initializer_list<std::string_view> columns = {
    ...
    "jetton_master_address", "response_destination", "custom_payload", "forward_ion_amount", "forward_payload", "trace_id"
  };
...
void InsertManagerPostgres::start_up() {
  ...
  if (!db_version.has_value()) {
    LOG(ERROR) << "Database version is not set, run `ion-index-postgres-migrate` to prepare the database";
    std::_Exit(2);
  }
  if (*db_version != latest_version) {
    LOG(ERROR) << "Database version mismatch: expected " << latest_version.str() << ", got " << db_version->str();
    LOG(ERROR) << "Run `ion-index-postgres-migrate` to update the database schema";
    std::_Exit(2);
  }
