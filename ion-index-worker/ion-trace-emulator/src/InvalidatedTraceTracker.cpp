...
void InvalidatedTraceTracker::register_pending_block(ion::BlockIdExt block_id_ext) {
...
void InvalidatedTraceTracker::add_confirmed_trace(ion::BlockIdExt block_id_ext, td::Bits256 ext_in_hash_norm) {
...
void InvalidatedTraceTracker::add_finalized_trace(ion::BlockIdExt block_id_ext, td::Bits256 ext_in_hash_norm) {
...
void InvalidatedTraceTracker::finalized_mc_block_emulated(std::vector<ion::BlockId> block_ids) {
    for (auto& block_id : block_ids) {
        process_finalized_block(block_id);
    }
}

void InvalidatedTraceTracker::confirmed_block_discarded(const ion::BlockIdExt& pending_block, const ion::BlockIdExt& finalized_block) {
    LOG(WARNING) << "Confirmed block " << pending_block.to_str() << " was discarded by finalized block "
                 << finalized_block.to_str();
}

void InvalidatedTraceTracker::process_finalized_block(const ion::BlockId& block_id) {
