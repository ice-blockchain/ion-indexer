...
public:
    ...
    void add_shard_state(const ion::BlockId& blkid, uint32_t timestamp, ion::LogicalTime lt, td::Ref<vm::Cell> cell) {
    ...
    td::Result<block::Account> get_account_state(const block::StdAddress& address) {
        {
            ...
            if (range.first != range.second) {
                ...
                for (auto& shard_state : shard_states_) {
                    if (ion::shard_contains(shard_state.blkid.shard_full(), ion::extract_addr_prefix(address.workchain, address.addr))) {
    ...
    ion::BlockSeqno get_mc_seqno() const {
        return mc_seqno_;
    }
        ...
        for (const auto &shard_state : shard_states_) {
            ...
            if (account.is_null()) {
                ...
                if (!account_state.unpack(std::move(account), shard_state.timestamp, 
                            address.workchain == ion::masterchainId && config_->is_special_smartcontract(address.addr))) {
