...
class TraceEmulatorScheduler : public td::actor::Actor {
  private: 
    ...
    ion::BlockSeqno last_known_seqno_{0};
    ion::BlockSeqno last_fetched_seqno_{0};
    ion::BlockSeqno last_emulated_seqno_{0};

    td::Timestamp next_statistics_flush_;

    std::unordered_set<ion::BlockSeqno> seqnos_to_fetch_;
    std::map<ion::BlockSeqno, MasterchainBlockDataState> blocks_to_emulate_;
    std::deque<ion::BlockIdExt> signed_block_queue_;
    std::unordered_set<ion::BlockIdExt, BlockIdExtHasher> signed_blocks_inflight_;
    std::unordered_map<ion::BlockIdExt, BlockDataState, BlockIdExtHasher> signed_block_storage_;
    ...
    void handle_block_signed(ion::BlockIdExt block_id);
    void handle_block_applied(ion::BlockIdExt block_id);

    void got_last_mc_seqno(ion::BlockSeqno last_known_seqno);
    void fetch_seqnos();
    ...
    void emulate_blocks();
    void enqueue_signed_block(ion::BlockIdExt block_id);
    void signed_block_fetched(ion::BlockIdExt block_id, BlockDataState block_data_state);
    void signed_block_error(ion::BlockIdExt block_id, td::Status error);
    void process_signed_blocks();
    std::function<void(Trace, td::Promise<td::Unit>, MeasurementPtr)> make_signed_trace_processor(const ion::BlockIdExt& block_id_ext);
  ...
  public:
    ...
    void handle_db_event(ion::tl_object_ptr<ion::ton_api::db_Event> event);
};
