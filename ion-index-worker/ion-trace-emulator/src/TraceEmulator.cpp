...
void TraceEmulatorImpl::emulate(td::Ref<vm::Cell> in_msg, block::StdAddress address, ion::LogicalTime lt, size_t depth,
    ...
    if (trans.outmsg_cnt > 0 && depth < context_.trace_depth_limit() && !context_.is_limit_exceeded()) {
        ...
        for (int ind = 0; ind < trans.outmsg_cnt; ind++) {
            ...
            if (!ion::shard_contains(block_id_.shard_full(), ion::extract_addr_prefix(out_msg_address.workchain, out_msg_address.addr))) {
...
void MasterchainBlockEmulator::start_up() {
    ...
    struct Bucket { ion::BlockId blkid; std::vector<EmulationMessage> msgs; };
    ...
    for (const auto& emulation_msg : in_msgs_) {
        ...
        for (const auto& shard_state : context_.get_shard_states()) {
            if (ion::shard_contains(shard_state.blkid.shard_full(),
                                    ion::extract_addr_prefix(msg_dest.workchain, msg_dest.addr))) {
