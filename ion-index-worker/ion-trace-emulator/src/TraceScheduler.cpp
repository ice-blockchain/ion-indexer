...
namespace {
constexpr const char* kHealthKey = "health:ion-trace-emulator";
constexpr auto kHealthTtl = std::chrono::seconds(20);
constexpr double kHealthIntervalSec = 1.0;
}  // namespace


void TraceEmulatorScheduler::handle_db_event(ion::tl_object_ptr<ion::ton_api::db_Event> event) {
    ion::ton_api::downcast_call(
        *event, td::overloaded(
                    [&](ion::ton_api::db_event_blockCandidateReceived &ev) {
                    },
                    [&](ion::ton_api::db_event_blockApplied &ev) {
                        LOG(WARNING) << "db_event_blockApplied: "<< ion::create_block_id(ev.block_id_).to_str();
                        handle_block_applied(ion::create_block_id(ev.block_id_));
                    },
                    [&](ion::ton_api::db_event_blockSigned &ev) {
                        LOG(WARNING) << "db_event_blockSigned: " << ion::create_block_id(ev.block_id_).to_str();
                        handle_block_signed(ion::create_block_id(ev.block_id_));
                    }));
}

void TraceEmulatorScheduler::handle_block_signed(ion::BlockIdExt block_id) {
...
void TraceEmulatorScheduler::handle_block_applied(ion::BlockIdExt block_id) {
    if (!block_id.is_masterchain()) {
        return;
    }
    auto P = td::PromiseCreator::lambda([SelfId = actor_id(this), block_id](td::Result<td::Unit> R) {
...
void TraceEmulatorScheduler::got_last_mc_seqno(ion::BlockSeqno new_last_known_seqno) {
...
void TraceEmulatorScheduler::enqueue_signed_block(ion::BlockIdExt block_id) {
...
void TraceEmulatorScheduler::signed_block_fetched(ion::BlockIdExt block_id, BlockDataState block_data_state) {
...
void TraceEmulatorScheduler::signed_block_error(ion::BlockIdExt block_id, td::Status error) {
    signed_blocks_inflight_.erase(block_id);
    LOG(ERROR) << "Failed to collect signed shard block " << block_id.to_str() << ": " << error;
    ion::delay_action([SelfId = actor_id(this), block_id]() {
        td::actor::send_closure(SelfId, &TraceEmulatorScheduler::enqueue_signed_block, block_id);
    }, td::Timestamp::in(0.1));
}
...
std::function<void(Trace, td::Promise<td::Unit>, MeasurementPtr)> TraceEmulatorScheduler::make_signed_trace_processor(const ion::BlockIdExt& block_id_ext) {
...
std::function<void(Trace, td::Promise<td::Unit>, MeasurementPtr)> TraceEmulatorScheduler::make_finalized_trace_processor(const MasterchainBlockDataState& mc_data_state) {
    std::unordered_map<ion::BlockId, ion::BlockIdExt, BlockIdHasher, BlockIdEq> shard_block_ids;
...
void TraceEmulatorScheduler::alarm() {
    if (db_event_fifo_path_.empty()) {
        auto P = td::PromiseCreator::lambda([SelfId = actor_id(this)](td::Result<ion::BlockSeqno> R){
            if (R.is_error()) {
    ...
    if (next_statistics_flush_.is_in_past()) {
        ion::delay_action([working_dir = this->working_dir_]() {
