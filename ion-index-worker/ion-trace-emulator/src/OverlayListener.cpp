...
void OverlayListener::start_up() {
    ...
    public:
        ...
        void receive_message(ion::adnl::AdnlNodeIdShort src, ion::overlay::OverlayIdShort overlay_id, td::BufferSlice data) override {
        }
        void receive_query(ion::adnl::AdnlNodeIdShort src, ion::overlay::OverlayIdShort overlay_id, td::BufferSlice data,
                        td::Promise<td::BufferSlice> promise) override {
            auto B = ion::fetch_tl_object<ion::ton_api::tonNode_getCapabilities>(std::move(data), true);
            if (B.is_error()) {
                promise.set_error(td::Status::Error("not implemented"));
                return;
            }
            
            promise.set_value(ion::create_serialize_tl_object<ion::ton_api::tonNode_capabilities>(0, 0, 0));
        }
        void receive_broadcast(ion::PublicKeyHash src, ion::overlay::OverlayIdShort overlay_id, td::BufferSlice data) override {
            auto B = ion::fetch_tl_object<ion::ton_api::tonNode_externalMessageBroadcast>(std::move(data), true);
            if (B.is_error()) {
                LOG(WARNING) << "Failed to fetch externalMessageBroadcast";
                return;
            }
            auto msg_data = std::move(B.move_as_ok()->message_->data_);
            auto message_r = ion::validator::ExtMessageQ::create_ext_message(std::move(msg_data), block::SizeLimitsConfig::ExtMsgLimits{});
            if (message_r.is_ok()) {
                auto message = message_r.move_as_ok();
                td::actor::send_closure(parent_, &OverlayListener::process_external_message, std::move(message));
            }
        }
        void check_broadcast(ion::PublicKeyHash src, ion::overlay::OverlayIdShort overlay_id, td::BufferSlice data,
    ...
    if (ion::ton_api::from_json(conf, conf_json.get_object()).is_error()) {
    ...
    auto dht_config = ion::dht::Dht::create_global_config(std::move(conf.dht_)).move_as_ok();

    td::Bits256 zero_state_file_hash = conf.validator_->zero_state_->file_hash_;
    auto X = ion::create_hash_tl_object<ion::ton_api::tonNode_shardPublicOverlayId>(0, ion::shardIdAll, zero_state_file_hash);
    td::BufferSlice b{32};
    b.as_slice().copy_from(as_slice(X));
    auto overlay_id_full = ion::overlay::OverlayIdFull{std::move(b)};
    auto overlay_id = overlay_id_full.compute_short_id();
    auto rules = ion::overlay::OverlayPrivacyRules{ion::overlay::Overlays::max_fec_broadcast_size()};
    ...
    adnl_network_manager_ = ion::adnl::AdnlNetworkManager::create(td::narrow_cast<td::uint16>(addr.get_port()));
    ion::adnl::AdnlCategoryMask cat_mask;
    cat_mask[0] = true;
    td::actor::send_closure(adnl_network_manager_, &ion::adnl::AdnlNetworkManager::add_self_addr, addr, std::move(cat_mask), 0);
    ...
    ion::adnl::AdnlAddress x = ion::adnl::AdnlAddressImpl::create(
        ion::create_tl_object<ion::ton_api::adnl_address_udp>(addr.get_ipv4(), addr.get_port()));
    ...
    td::actor::send_closure(adnl_, &ion::adnl::Adnl::add_id, ion::adnl::AdnlNodeIdFull{pub}, std::move(addr_list),
                        static_cast<td::uint8>(0));

    dht_ = ion::dht::Dht::create(ion::adnl::AdnlNodeIdShort{pub.compute_short_id()}, "",
            dht_config, keyring_.get(), adnl_.get()).move_as_ok();

    td::actor::send_closure(adnl_, &ion::adnl::Adnl::register_dht_node, dht_.get());

    overlays_ = ion::overlay::Overlays::create("", keyring_.get(), adnl_.get(), dht_.get());
    td::actor::send_closure(overlays_, &ion::overlay::Overlays::create_public_overlay, ion::adnl::AdnlNodeIdShort{pub.compute_short_id()}, overlay_id_full.clone(),
                            std::make_unique<Callback>(actor_id(this)), rules, PSTRING() << "{ \"type\": \"shard\", \"shard_id\": " << ion::shardIdAll << ", \"workchain_id\": " << 0 << " }");
}

void OverlayListener::process_external_message(td::Ref<ion::validator::ExtMessageQ> message) {
