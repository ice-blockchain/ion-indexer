...
void IntegrityChecker::got_last_mc_seqno(ion::BlockSeqno last_known_seqno) {
...
void IntegrityChecker::got_min_mc_seqno(ion::BlockSeqno min_known_seqno) {
    LOG(INFO) << "Oldest DB known seqno: " << min_known_seqno;

    find_oldest_seqno_with_state(min_known_seqno, to_seqno_);
}

void IntegrityChecker::got_block_handle(ion::validator::ConstBlockHandle handle) {
    ...
    if (handle->inited_state_boc() && !handle->deleted_state_boc()) {
        ...
        auto P = td::PromiseCreator::lambda([SelfId = actor_id(this)](td::Result<ion::validator::ConstBlockHandle> R){
            if (R.is_error()) {
...
void IntegrityChecker::find_oldest_seqno_with_state(uint32_t min_seqno, uint32_t max_seqno) {
    ...
    auto P = td::PromiseCreator::lambda([SelfId = actor_id(this), min_seqno, max_seqno, mid_seqno](td::Result<ion::validator::ConstBlockHandle> R){
        if (R.is_error()) {
            LOG(ERROR) << "Failed to get block handle: " << R.move_as_error();
            std::_Exit(2);
        }
...
void IntegrityChecker::got_oldest_mc_seqno_with_state(ion::BlockSeqno oldest_seqno_with_state) {
...
void IntegrityChecker::fetch_next_seqnos() {
    if (blocks_to_parse_.size() > blocks_to_parse_queue_max_size_) {
        ...
        ion::delay_action([SelfId = actor_id(this)]() { td::actor::send_closure(SelfId, &IntegrityChecker::fetch_next_seqnos); }, td::Timestamp::in(1.0));
        return;
    }
...
void IntegrityChecker::alarm() {
    ...
    // Save to checkpoint last seqno up to which we have verified the DB (seqnos_processed_ sequence is NOT guaranteed to be monoionically increasing)
    if (checkpoint_path_.size()) {
