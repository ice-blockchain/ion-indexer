...
struct TraceImpl {
    ...
    ion::BlockSeqno mc_seqno_start;
    ion::BlockSeqno mc_seqno_end;
    ...
    // methods
    TraceImpl() {}
    TraceImpl(ion::BlockSeqno seqno, const schema::Transaction &tx) :
...
class TraceAssembler: public td::actor::Actor {
    ...
    std::string db_path_;
    size_t gc_distance_;
    ion::BlockSeqno expected_seqno_{0};
    std::map<ion::BlockSeqno, Task> queue_;
...
public:
    ...
    void assemble(ion::BlockSeqno mc_seqno, ParsedBlockPtr mc_block_, td::Promise<ParsedBlockPtr> promise);
    
    td::Result<ion::BlockSeqno> restore_state(ion::BlockSeqno expected_seqno);
    void reset_state();
    void set_expected_seqno(ion::BlockSeqno expected_seqno);
    void start_up() override;
    void alarm() override;
private:
    void process_queue();
    void process_block(ion::BlockSeqno seqno, ParsedBlockPtr block);
    void process_transaction(ion::BlockSeqno seqno, schema::Transaction& tx, std::vector<TraceEdgeImpl>& edges_found_, 
