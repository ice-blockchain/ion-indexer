...
td::Status gc_states(std::string db_path, ion::BlockSeqno current_seqno, size_t keep_last) {
    td::Timer timer;
...
td::Status save_state(std::string db_path, ion::BlockSeqno seqno, 
...
void TraceAssembler::alarm() {
    ...
    ion::delay_action([db_path = this->db_path_, seqno = expected_seqno_ - 1, pending_traces = pending_traces_, pending_edges = pending_edges_]() {
...
void TraceAssembler::assemble(ion::BlockSeqno seqno, ParsedBlockPtr block, td::Promise<ParsedBlockPtr> promise) {
...
void TraceAssembler::set_expected_seqno(ion::BlockSeqno new_expected_seqno) { 
...
td::Result<ion::BlockSeqno> TraceAssembler::restore_state(ion::BlockSeqno seqno) {
    std::map<int, fs::path, std::greater<int>> fileMap;
    try {
    ...
    return td::Status::Error(ion::ErrorCode::warning, "TraceAssembler state not found");    
}
...
void TraceAssembler::process_block(ion::BlockSeqno seqno, ParsedBlockPtr block) {
...
void TraceAssembler::process_transaction(ion::BlockSeqno seqno, schema::Transaction& tx, std::vector<TraceEdgeImpl>& completed_edges, 
