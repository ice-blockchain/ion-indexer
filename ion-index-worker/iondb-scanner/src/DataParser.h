...
private:
  ...
  schema::Block parse_block(const td::Ref<vm::Cell>& root_cell, const ion::BlockIdExt& blk_id, block::gen::Block::Record& blk, const block::gen::BlockInfo::Record& info, 
                            const block::gen::BlockExtra::Record& extra, const td::optional<schema::Block> &mc_block);
  schema::MasterchainBlockShard parse_shard_state(const schema::Block& mc_block, const ion::BlockIdExt& shard_blk_id);
  ...
  td::Result<std::vector<schema::Transaction>> parse_transactions(const ion::BlockIdExt& blk_id, const block::gen::Block::Record &block, 
  ...
  td::Result<std::vector<schema::AccountState>> parse_account_states_new(ion::WorkchainId workchain_id, uint32_t gen_utime, std::map<td::Bits256, AccountStateShort> &account_states);
