...
private:
  td::actor::ActorId<ion::validator::RootDb> db_;
...
public:
  GetBlockDataState(td::actor::ActorId<ion::validator::RootDb> db, ConstBlockHandle handle, td::Promise<BlockDataState> promise) :
    db_(db),
    handle_(handle),
...
private:
  ...
  std::queue<ion::BlockIdExt> blocks_queue_;

  std::set<ion::BlockIdExt> current_shard_blk_ids_;
  std::unordered_set<ConstBlockHandle> shard_block_handles_;

  MasterchainBlockDataState result_;

public:
  IndexQuery(int mc_seqno, td::actor::ActorId<ion::validator::RootDb> db, td::Promise<MasterchainBlockDataState> promise) : 
    db_(db), 
    mc_seqno_(mc_seqno),
    promise_(std::move(promise)) {
  }
  ...
  void start_up() override {
    ...
    td::actor::send_closure(db_, &RootDb::get_block_by_seqno, ion::AccountIdPrefixFull(ion::masterchainId, ion::shardIdAll), mc_seqno_, std::move(P));
  }
...
void DbScanner::start_up() {
  auto opts = ion::validator::ValidatorManagerOptions::create(
  ...
  db_ = td::actor::create_actor<ion::validator::RootDb>("db", td::actor::ActorId<ion::validator::ValidatorManager>(), db_root_, std::move(opts), mode);

  alarm_timestamp() = td::Timestamp::in(1.0);
}

void DbScanner::get_last_mc_seqno(td::Promise<ion::BlockSeqno> promise) {
  td::actor::send_closure(db_, &RootDb::get_max_masterchain_seqno, std::move(promise));
}

void DbScanner::get_oldest_mc_seqno(td::Promise<ion::BlockSeqno> promise) {
  td::actor::send_closure(db_, &RootDb::get_min_masterchain_seqno, std::move(promise));
}

void DbScanner::get_mc_block_handle(ion::BlockSeqno seqno, td::Promise<ion::validator::ConstBlockHandle> promise) {
  td::actor::send_closure(db_, &RootDb::get_block_by_seqno, ion::AccountIdPrefixFull(ion::masterchainId, ion::shardIdAll), seqno, std::move(promise));
}
...
void DbScanner::iterate_temp_block_handles(std::function<void(const ion::validator::BlockHandleInterface&)> f) {
  td::actor::send_closure(db_, &RootDb::iterate_temp_block_handles, std::move(f));
}

void DbScanner::fetch_block_by_id(ion::BlockIdExt block_id, td::Promise<BlockDataState> promise) {
  auto P = td::PromiseCreator::lambda(
      [SelfId = actor_id(this), this, promise = std::move(promise)](td::Result<ion::validator::BlockHandle> R) mutable {
        if (R.is_error()) {
