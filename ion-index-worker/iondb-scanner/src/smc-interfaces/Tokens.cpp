...
void JettonWalletDetectorR::verify_with_master(td::Ref<vm::Cell> master_code, td::Ref<vm::Cell> master_data, Result jetton_wallet_data) {
  ion::SmartContract smc({master_code, master_data});
...
const auto dot_ion_dns_root_addr_mainnet = block::StdAddress::parse("0:B774D95EB20543F186C06B371AB88AD704F7E256130CAF96189368A7D0CB6CCF").move_as_ok();
const auto dot_ion_dns_root_addr_testnet = block::StdAddress::parse("0:E33ED33A42EB2032059F97D90C706F8400BB256D32139CA707F1564AD699C7DD").move_as_ok();
...
block::StdAddress NftItemDetectorR::get_dot_ion_dns_root_addr() {
  if (is_testnet) {
    return dot_ion_dns_root_addr_testnet;
  }
  return dot_ion_dns_root_addr_mainnet;
}
...
void NftItemDetectorR::got_collection(Result item_data, td::Ref<vm::Cell> ind_content, td::Ref<vm::Cell> collection_code, td::Ref<vm::Cell> collection_data) {
  ...
  process_domain_and_dns_data(get_dot_ion_dns_root_addr(), [this](){ return this->get_ion_domain(); }, item_data);
...
td::Result<std::string> NftItemDetectorR::get_ion_domain() {
...
bool DedustPoolDetector::get_asset(td::Ref<vm::CellSlice> slice, std::optional<block::StdAddress> &address) {
  ...
  if (sum_type == 0) { // ION asset
    address = std::nullopt;
    return true;
  }
  if (sum_type == 1) {
    const auto remaining_bits = cell_slice.data_bits();
    const auto workchain_id = (ion::WorkchainId)remaining_bits.get_int(8);
    const td::ConstBitPtr addr_ptr = remaining_bits + 8;
    const ion::StdSmcAddress addr_bytes{addr_ptr};
