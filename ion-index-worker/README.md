...
This is a C++ worker for [ION indexer](https://github.com/ice-blockchain/ion-indexer/). This worker reads data from ION node files, parses TL-B schemas, and inserts data into PostgreSQL database.

## 1. How to install

Before installing the worker, ensure the ION Index database is set up using the [instructions provided](https://github.com/ice-blockchain/ion-indexer/tree/master).
...
2. Build ION index worker binary:
        ...
        ninja -j$(nproc) ion-index-postgres
...
* `--dbname <dbname>` - PostgreSQL database name. Default: `ion_index`.
