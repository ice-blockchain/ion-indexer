...
import (
	...
	"ion-metadata-cache/cache"
	"ion-metadata-cache/repl"
)
...
func TestDnsEntries_OwnerChangeCleanup(t *testing.T) {
	...
	// Verify: ALICE should have domain "alice.ion"
	domain := h.GetShortestDomain(ctx, "ALICE")
	if domain != "alice.ion" {
		t.Errorf("expected domain 'alice.ion', got '%s'", domain)
	}
	...
	if domain != "alice.ion" {
		t.Errorf("expected BOB's domain 'alice.ion', got '%s'", domain)
	}
}
...
func TestDnsEntries_SelfResolvingOnly(t *testing.T) {
	...
	if *entry.Domain != "test.ion" {
		t.Errorf("expected domain 'test.ion', got '%s'", *entry.Domain)
	}
}
...
func TestDnsEntries_ShortestDomainPreference(t *testing.T) {
	...
	// INSERT first domain for ALICE: "alice.ion" (10 chars)
	...
	// INSERT second domain for ALICE: "a.ion" (5 chars - shorter)
	...
	domain := h.GetShortestDomain(ctx, "ALICE")
	if domain != "a.ion" {
		t.Errorf("expected shortest domain 'a.ion', got '%s'", domain)
	}

	// INSERT third domain for ALICE: "verylongalice.ion" (longer)
	...
	domain = h.GetShortestDomain(ctx, "ALICE")
	if domain != "a.ion" {
		t.Errorf("expected domain to remain 'a.ion', got '%s'", domain)
	}
}
...
func TestDnsEntries_OwnerBecomesNonSelfResolving(t *testing.T) {
	...
	domain := h.GetShortestDomain(ctx, "ALICE")
	if domain != "alice.ion" {
		t.Fatalf("expected domain 'alice.ion', got '%s'", domain)
	}
...
func TestDnsEntries_TransferShortestKeepsBackup(t *testing.T) {
	...
	// ALICE has two domains: "a.ion" (shortest) and "alice.ion"
	event1 := repl.ChangeEvent{
		Table:     "public.dns_entries",
		Operation: repl.Insert,
		Data: map[string]interface{}{
	...
	domain := h.GetShortestDomain(ctx, "ALICE")
	if domain != "a.ion" {
		t.Fatalf("expected 'a.ion', got '%s'", domain)
	}

	// Transfer "a.ion" to BOB
	...
	// Verify: BOB now has "a.ion"
	domain = h.GetShortestDomain(ctx, "BOB")
	if domain != "a.ion" {
		t.Errorf("expected BOB to have 'a.ion', got '%s'", domain)
	}

	// KEY TEST: ALICE should now show "alice.ion" (her remaining domain)
	domain = h.GetShortestDomain(ctx, "ALICE")
	if domain != "alice.ion" {
		t.Errorf("expected ALICE to fall back to 'alice.ion', got '%s'", domain)
	}
}
