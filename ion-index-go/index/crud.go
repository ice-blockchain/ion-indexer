...
func buildTransactionsQuery(
	...
	// filters
	excludedAccounts := os.Getenv("ION_ACCOUNT_EXCLUDED")
	if excludedAccounts != "" {
...
func buildJettonTransfersQuery(transfer_req JettonTransferRequest, utime_req UtimeRequest,
	...
	clmn_query := `T.tx_hash, T.tx_lt, T.tx_now, T.tx_aborted, T.query_id,
		...
		T.forward_ion_amount, T.forward_payload, T.trace_id`
...
func buildActionsQuery(act_req ActionRequest, utime_req UtimeRequest, lt_req LtRequest, lim_req LimitRequest, settings RequestSettings) (string, []any, error) {
	...
	clmn_query_default := `A.trace_id, A.action_id, A.start_lt, A.end_lt, A.start_utime, A.end_utime, 
		...
		A.type, (A.ion_transfer_data).content, (A.ion_transfer_data).encrypted, A.value, A.amount,
	...
	if v := act_req.AccountAddress; v != nil && len(*v) > 0 {
		filter_str := fmt.Sprintf("AA.account = '%s'::ionaddr", *v)
		filter_list = append(filter_list, filter_str)
	...
	if v := act_req.TransactionHash; v != nil {
		...
		from_query = `actions as A join transactions as T on A.trace_id = T.trace_id and A.tx_hashes @> array[T.hash::ionhash]`
		if order_by_now {
	...
	if v := act_req.MessageHash; len(v) > 0 {
		...
		from_query = `actions as A join messages as M on A.trace_id = M.trace_id and array[M.tx_hash::ionhash] <@ A.tx_hashes`
		if order_by_now {
			clmn_query = `distinct on (A.trace_end_utime, A.trace_id, A.end_utime, A.action_id) ` + clmn_query_default
		} else {
			clmn_query = `distinct on (A.trace_end_lt, A.trace_id, A.end_lt, A.action_id) ` + clmn_query_default
	...
	if len(trace_id_list) > 0 {
		if includeActions {
			...
			A.value_extra_currencies,
				...
				(A.ionco_deploy_pool_data).jetton0_router_wallet,
				(A.ionco_deploy_pool_data).jetton1_router_wallet,
				(A.ionco_deploy_pool_data).jetton0_minter,
				(A.ionco_deploy_pool_data).jetton1_minter,
				(A.ionco_deploy_pool_data).tick_spacing,
				(A.ionco_deploy_pool_data).initial_price_x96,
				(A.ionco_deploy_pool_data).protocol_fee,
				(A.ionco_deploy_pool_data).lp_fee_base,
				(A.ionco_deploy_pool_data).lp_fee_current,
				(A.ionco_deploy_pool_data).pool_active,
				(A.coffee_create_pool_data).amount_1,
				(A.coffee_create_pool_data).amount_2,
...
func buildMultisigQuery(multisig_req MultisigRequest, lim_req LimitRequest, settings RequestSettings) (string, error) {
	...
	if len(multisig_req.WalletAddress) > 0 {
		...
		conditions = append(conditions, fmt.Sprintf("(m.signers && ARRAY[%s]::ionaddr[] OR m.proposers && ARRAY[%s]::ionaddr[])",
