...
/*
#cgo CPPFLAGS: -I${SRCDIR}/../../ion-index-worker/ion-marker/src
#cgo LDFLAGS: -L${SRCDIR}/../../build/ion-index-worker/ion-marker -lion-marker-core -lion-marker -Wl,-rpath,${SRCDIR}/../../build/ion-index-worker/ion-marker
...
// just a simple check
func init() {
	fmt.Println("initializing ion-marker library...")
	result := C.ion_marker_decode_opcode(C.uint(0))
	if result != nil {
		isLibraryInitialized = true
		C.ion_marker_free_string(result)
		fmt.Println("ion-marker library initialized successfully")
	} else {
		fmt.Println("warning: ion-marker library initialization failed")
	}
}
...
func MarkerRequest(opcodesList []uint32, bocBase64List []string) ([]string, []string, error) {
	if !isLibraryInitialized {
		return nil, nil, errors.New("ion-marker library is not initialized")
	}
	...
	// call batch function
	response := C.ion_marker_process_batch(&request)
	if response == nil {
		return nil, nil, errors.New("ion_marker_process_batch failed or returned nil")
	}

	defer func() {
		if response != nil {
			C.ion_marker_free_batch_response(response)
		}
	}()
