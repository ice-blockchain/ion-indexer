...
import (
	...
	"github.com/ice-blockchain/ion-indexer/ion-index-go/index/emulated"
	...
	_ "github.com/ice-blockchain/ion-indexer/ion-index-go/docs"
	"github.com/ice-blockchain/ion-indexer/ion-index-go/index"
)
...
//
//		 **Note:** this method is not equivalent with [/api/v2/shards](https://ice-blockchain.com/api/v2/#/blocks/get_shards_shards_get).
//
...
//
// @description Query DNS records by specified filters. Currently .ion and .t.me DNS are supported.
//
...
// @router /indexer/v3/nft/sales [get]
// @security APIKeyHeader
// @security APIKeyQuery
...
func GetTopAccountsByBalance(c *fiber.Ctx) error {
	...
	return c.JSON(FilterAccountBalances(res))
}
...
// @param action_type query []string false "Include action types." Enums(call_contract, contract_deploy, ion_transfer, auction_bid, change_dns, dex_deposit_liquidity, dex_withdraw_liquidity, delete_dns, renew_dns, election_deposit, election_recover, jetton_burn, jetton_swap, jetton_transfer, jetton_mint, nft_mint, tick_tock, stake_deposit, stake_withdrawal, stake_withdrawal_request, subscribe, unsubscribe)
// @param exclude_action_type query []string false "Exclude action types." Enums(call_contract, contract_deploy, ion_transfer, auction_bid, change_dns, dex_deposit_liquidity, dex_withdraw_liquidity, delete_dns, renew_dns, election_deposit, election_recover, jetton_burn, jetton_swap, jetton_transfer, jetton_mint, nft_mint, tick_tock, stake_deposit, stake_withdrawal, stake_withdrawal_request, subscribe, unsubscribe)
// @param supported_action_types query []string false "Supported action types"
...
// @router /indexer/v3/multisig/wallets [get]
// @security		APIKeyHeader
// @security		APIKeyQuery
...
// @router /indexer/v3/multisig/orders [get]
// @security		APIKeyHeader
// @security		APIKeyQuery
...
func FilterActions(actions []index.Action) []index.Action {
	...
	for _, a := range actions {
		...
		case *index.ActionDetailsIonTransfer:
			if d.Source != nil {
...
func GetExcludedAccounts() []string {
	env := os.Getenv("ION_ACCOUNT_EXCLUDED")
	if env == "" {
		return nil
	}
...
func main() {
	...
	app.Get("/indexer/v3/dns/records", GetDNSRecords)

	// vesting
	app.Get("/indexer/v3/vesting", GetVestingContracts)
	...
	app.Get("/indexer/v3/nft/sales", GetNFTSales)
	...
	// multisig
	app.Get("/indexer/v3/multisig/wallets", GetMultisigs)
	app.Get("/indexer/v3/multisig/orders", GetMultisigOrders)
	...
	// utils
	app.Get("/indexer/v3/decode", GetDecode)
	app.Post("/indexer/v3/decode", PostDecode)
